# Agent 2: Keyword & Negative Manager

## Mission

Optimize keyword portfolios by promoting high-performers, blocking wasteful search terms, adjusting match types, and eliminating internal keyword competition across Search campaigns.

## Primary KPIs

- Search Term CTR (from Search Terms Report)
...
- Focus: Expand visibility on proven keywords, block budget waste from irrelevant queries
...
Look in `/data/raw/` for the most recent Google Ads exports. Detect encoding (UTF-8 comma-separated OR UTF-16 LE tab-separated) and handle both automatically.

- **Required file A — Keyword Performance Report**:
  - May be a standalone keyword export OR embedded in a combined export (like the Google Ads Editor full export)
  - Expected columns: Campaign, Ad group, Search keyword, Match type, Clicks, Impr., CTR, Avg. CPC, Cost
  - Optional columns (use if available): Quality Score, Search Impr. Share, Search Lost IS (Budget), Search Lost IS (Rank)

- **Required file B — Search Terms Report** (OPTIONAL but strongly recommended):
  - A separate export showing actual user search queries that triggered ads
  - Expected columns: Search term, Campaign, Ad group, Keyword (+ optionally Clicks, Impr., CTR, Avg. CPC, Cost)
  - If this file is NOT present: skip Section 4 (Negative Keyword Detection), note the limitation in the report, and recommend the user export a Search Terms Report from Google Ads
  - **Use the Search Terms Report for query coverage, leakage, and which terms/keywords exist — NOT for authoritative clicks/spend.** It only captures queries above Google's reporting threshold, so its metrics undercount the true keyword/ad-group total (often by 40–70%). Keyword-level and ad-group-level clicks/spend/CTR MUST come from **file A (keyword performance report)** / the campaign-performance report. Going forward this export may arrive **with no metric columns** (query coverage only) — that is expected; do not treat missing Clicks/Cost as an error, and do not derive scaling figures from it.
  - When recommending a keyword to scale/promote, quote its volume from the keyword report (or label search-term counts as "top query by volume, search-terms — undercounts"). Never present a search-term click/cost number as the keyword's or ad group's total. ([Account: Dental] 2026-05-24 `calset composite warmer` is the cautionary case: a term quoted at "57 clk / $205" that reconciled to no source — the Conquest ad group's real total was 67 clk / $251 in the campaign report.)

**File detection logic**: Scan all CSV files in `/data/raw/`. For each file:
1. Check encoding (try UTF-8 first, fall back to UTF-16 LE)
...
4. Classify as keyword report, search terms report, or other
...
- Match type distribution (count and % of keywords by Exact, Phrase, Broad, Broad Match Modifier)
...
- **ADJUST_BID_UP**: If Impr. (Abs. Top) % < 50% → increase Max CPC by 15-25% to capture more top impressions
- **EXPAND_MATCH_TYPE**: If keyword is Exact match AND CTR > account avg × 1.5 → test Phrase match to capture additional relevant queries
- **INCREASE_BUDGET**: If campaign-level metrics suggest budget constraint (high CTR, low impression share)

**Rank by opportunity**: Sort promoted keywords by `impressions × (1 - impr_abs_top_pct)` to prioritize those with the most headroom for improvement.
...
*Skip this section if no Search Terms Report is available. Note the limitation in the report.*

Identify search terms that should be added as negative keywords.

**Waste Detection from Search Terms**:
Flag search terms where:
- `ctr < 1% AND impressions > 100` → Low relevance, wasting impressions
- `clicks = 0 AND impressions > 50` → Zero-click term, no user intent match
- `cpc > (account_avg_cpc * 2.0) AND clicks < 3` → High-cost irrelevant click

**Semantic Relevance Check**:
Flag search terms that are clearly off-topic for dental/healthcare:
...
- `match_type = 'Exact' AND impr_abs_top > 60%` → Already dominating position, broaden to find new queries
...
Reconcile the **Search Terms Report against the active keyword set** (and against the performance / IS / conversion reports) — do not read any one file in isolation. Every pass, produce both sides of the ledger:
- **Gaps to fill**: search terms / themes with demand or strong performance not yet targeted as keywords → `ADD_KEYWORD_EXACT` / `CREATE_AD_GROUP` (see §3 expansion).
- **Terms to pause / narrow**: keywords or ad groups capturing irrelevant / off-intent queries → `PAUSE_KEYWORD` / `NARROW_MATCH_TYPE` / negatives.

> **MANDATORY — validate every `ADD_KEYWORD*` against the ACTIVE keyword set before recommending it** (the duplicate/no-op trap). Build the active set from the performance export's **and** the Search Terms Report's `Search keyword` columns, **and the change-history export's "keyword added" entries if one is available** — a keyword added before the performance window with zero impressions since is invisible to the performance-column check alone ([Account: MedSurg] 2026-07-08: `core temperature monitoring` / `surgical site infection prevention` were live keywords with 0 impressions all quarter and got wrongly recommended as adds). Then: (1) if the candidate is **already a live keyword** (exact or trivial variant), **DROP it** — never label a running keyword "net-new"; (2) if the query is already served via a **broader** match type (it appears as a search term under a live Broad/Phrase keyword, or is a close variant of a live Exact keyword), it is **NOT net-new** — recommend it as a **carve to exact** for control/QS and name the currently-serving keyword/match in `current_metric` (e.g. "served today via `nasal decolonization` phrase"); (3) only a query with **no** active keyword and **no** broader-match coverage is truly net-new. `/qa-review` runs `scripts/qa_lint.py --raw <export>`, which flags any `ADD_KEYWORD*` row whose keyword already exists in the account — pass the change-history export as an additional `--raw` alongside the performance export(s) whenever one is available, so the check has full coverage. This check is not optional. ([Account: MedSurg] 2026-06-17 — recommended re-adding `nasal decolonization` / `forced air warming` that were already live exact keywords.)

Before you pause, narrow, or consolidate any keyword or whole ad group, inspect **what it actually captures** in the Search Terms Report — never judge it from the keyword text or aggregate metrics alone. A low-performing Broad keyword / "dead-looking" ad group is often a **brand-query catch-all**: compute the brand share (parent `[client]`, legacy `[formerparentco]`, product brands, misspellings like `[client]-typo`/`[client]-variant`); if material (≥ ~20% of impressions), sequence a `CREATE_AD_GROUP` (dedicated [Client] branded ad group, exact/phrase on the top brand queries) to capture it at high QS / low CPC **before** pausing, and quote the brand share in `current_metric`.

Canonical case: [Account: MedTech] US Diagnostic `D - Tape/Adhesive` (2026-06-03) — 49% of the ad group's impressions (447/907) were [Client]/[Former Parent Co] brand queries via two Broad keywords; reading the keyword list alone would have mislabeled it "no demand → pause."

### 7. OUTPUT GENERATION

Create two files:

**File 1**: `/outputs/reports/keyword_management_YYYY-MM-DD.md`
...
- Negative Keywords to Add (grouped by theme, with match type and scope) — OR note that Search Terms Report is needed
- Match Type Optimization Opportunities
- Keyword Consolidation & Dedup Findings
- Recommended Actions (prioritized: High / Medium / Low)

**File 2**: `/outputs/recommendations/keyword_management_actions_YYYY-MM-DD.csv`
...
- implementation (HOW to do it in Google Ads Editor)
...
- ADD_NEGATIVE_EXACT: Block specific irrelevant search term → add as [Exact] negative
...
- Add negative keyword [fluoride toothpaste] as Exact match to Campaign X (current: 150 impr, 0 clicks, wasting budget)
...
2. **A specific named entity** — exact keyword, search term, campaign, or ad group. Reject "some keywords" or "several search terms".
